...
- (void)setPictureModel:(SRPictureModel *)pictureModel {
    ...
    [self.pictureIndicator hide];
    
    if (pictureModel.picture) {
        self.imageView.image = pictureModel.picture;
        if (pictureModel.isFirstShow) {
            self.imageView.frame = pictureModel.originPosition;
            ...
            [UIView animateWithDuration:0.3 delay:0.0 usingSpringWithDamping:0.7 initialSpringVelocity:0.0 options:0 animations:^{
                self.imageView.frame = pictureModel.destinationPosition;
            } completion:nil];
        } else {
            self.imageView.frame = pictureModel.destinationPosition;
        }
        ...
        self.pictureIndicator = [SRPictureIndicator showInView:self];
...
- (void)handleSingleTap:(UITapGestureRecognizer *)gestureRecognizer {
    if (gestureRecognizer.numberOfTapsRequired == 1) {
        if ([self.pictureViewDelegate respondsToSelector:@selector(pictureViewDidTap)]) {
            [self.pictureViewDelegate pictureViewDidTap];
        }
    }
}
